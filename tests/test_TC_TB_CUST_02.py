...
@pytest.mark.parametrize(
    "customer_test_case",
    [get_test_case("customer_test_data.json", "invalid_customers", 0)]  # index 0 → Rownak Ahmed Oyshi
) #connecting data loader


def test_tc_tb_cust_02(browser_config,customer_test_case):
    logging.info("TC_TB_CUST_02 Started..")
    ...
    try:
        validation_message = customer_register_page.get_validation_message("#customerNameReg")
        expected_message = customer_test_case["expected_validation_message"]

        if validation_message == expected_message:
            logging.info(f"Test Passed. Customer Full name Validation message matched: '{validation_message}'")
        else:
            logging.info(f"Test Failed. Expected: '{expected_message}', Actual: '{validation_message}'")
            capture_full_page_screenshot(driver, "BUG_TC_TB_CUST_02")
            pytest.fail("Customer Full name Validation message did not match expected.")

    except Exception as e:
        logging.info("Customer Full name Validation message could not be fetched.")
        pytest.fail("Test Failed. Bug in fetching Customer Full name  validation message!")
